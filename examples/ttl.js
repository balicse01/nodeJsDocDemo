...
/**
 * This example demonstrates usage of Time-to-Live (TTL) feature of the 
 * Oracle NoSQL Database JS API.
 * 
 * The example:
 * - Creates a table.
 * - Puts a row in the table with a TTL value.
 * - Gets the row from the table and prints the row with its TTL value.
 */
...
store.open(function (err) {
  ...
  store.execute(' CREATE TABLE IF NOT EXISTS ' + TABLE_NAME +
    ...
    store.refreshTables(function (err) {
      ...
      // set updateTTL on WriteOptions
      writeOptions.updateTTL = true;
      console.log('Inserting row with TTL: ' + util.inspect(row));
          ...
          if (err)
            ...
            store.get(TABLE_NAME, pKey, readOptions,
              function (error, result) {
                ...
                console.log('Expiration (UTC) time: ' + result.expirationTime
                        + ' (' + toTTL(result.expirationTime) + ' from now)');
                // closing...
...
function toTTL(utc) {
    var duration = (utc - new Date().getTime())/1000;
